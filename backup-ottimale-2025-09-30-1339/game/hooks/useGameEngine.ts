...
function generateLanes(): LaneConfig[] {
  // 6 lanes from near-surface to deep, faster at top, monsters bottom
  ...
  return lanes.map((lane, i) => {
    ...
    const yPercent = 40 + i * 10; // water starts around 35-40%
    // Slightly slower overall movement
    const speedPxPerSec = 30 + (5 - i) * 8; // top lanes faster
    // Reduce spawn/density by increasing spacing
    const spacingPx = 160 - i * 5; // deeper lanes can be a bit denser
    return {
      id: i,
      yPercent,
      speedPxPerSec,
...
export function useGameEngine() {
  ...
  useEffect(() => {
    if (!started || over || hook) return; // stop oscillating when hook is active
    ...
    setFish(prev => {
      ...
      const getCatchRadiusForLane = (laneId: number) => {
        if (isPoweredHook) {
          ...
          // Lane 3: 90% radius  
          // Lane 4: 85% radius
          // Lane 5: 80% radius (monsters still challenging but much easier)
          const difficultyMultiplier = Math.max(0.8, 1 - (laneId * 0.05));
          ...
          const difficultyMultiplier = Math.max(0.4, 1 - (laneId * 0.15));
          return baseCatchRadius * difficultyMultiplier;
        }
      };
  ...
  useEffect(() => {
    ...
    // Check level completion - progressive targets
    const levelTargets = [0, 3000, 5000, 7000, 9000, 12000, 15000, 18000, 22000, 26000, 30000];
    const levelTarget = levelTargets[level] || 30000;
  ...
  }, [started, over, timeLeft, score, level, levelComplete]);
  ...
  }, [started, over, hook, indicatorDeg, bonus.active]);
